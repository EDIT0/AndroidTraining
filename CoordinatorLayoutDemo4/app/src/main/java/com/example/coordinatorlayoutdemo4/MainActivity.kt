...
import com.example.coordinatorlayoutdemo4.databinding.ActivityMainBinding
        ...
        binding.nestedScrollview.setOnScrollChangeListener(object : View.OnScrollChangeListener {
            override fun onScrollChange(p0: View?, p1: Int, scrollY: Int, p3: Int, p4: Int) {
                when {
                    ...
                    scrollY >= binding.nestedScrollview.computeDistanceToView(binding.tv1)  &&
                            scrollY < binding.nestedScrollview.computeDistanceToView(binding.tv2)
                    -> binding.tablayout.setScrollPosition(1, 0f, true)
                    scrollY >= binding.nestedScrollview.computeDistanceToView(binding.tv2) &&
                            scrollY < binding.nestedScrollview.computeDistanceToView(binding.tv3)
                    ->  binding.tablayout.setScrollPosition(2, 0f, true)
                    scrollY >= binding.nestedScrollview.computeDistanceToView(binding.tv3) &&
                            scrollY < binding.nestedScrollview.computeDistanceToView(binding.tv4)
                    ->   binding.tablayout.setScrollPosition(3, 0f, true)
                    scrollY >= binding.nestedScrollview.computeDistanceToView(binding.tv4) &&
                            scrollY < binding.nestedScrollview.computeDistanceToView(binding.tv5)
                    ->   binding.tablayout.setScrollPosition(4, 0f, true)
                    scrollY >= binding.nestedScrollview.computeDistanceToView(binding.tv5) &&
                            scrollY < binding.nestedScrollview.computeDistanceToView(binding.tv6)
                    ->   binding.tablayout.setScrollPosition(5, 0f, true)
                }

                if(!binding.nestedScrollview.canScrollVertically(1))  binding.tablayout.setScrollPosition(6, 0f, true)
            }
