...
class ThirdActivity : AppCompatActivity() {
    ...
    private val thirdViewModel = ThirdViewModel()
    ...
    fun observerViewModel() {
        thirdViewModel.str.observe(this) {
            Log.i("MYTAG", "ThirdActivity Thread Name: ${Thread.currentThread().name}")
            findViewById<TextView>(R.id.tvCount).text = "Count : ${it}"
            Log.i("MYTAG", "ThirdActivity 관찰자 ${it}")
        }
    }

    @Subscribe
    fun receiveEventBus(data: String) {
        Log.i("MYTAG", "ThirdActivity-receiveEventBus ${data}")
        thirdViewModel.setData(data)
    }
