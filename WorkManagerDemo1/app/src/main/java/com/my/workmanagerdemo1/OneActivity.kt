...
import com.my.workmanagerdemo1.databinding.ActivityOneBinding
...
class OneActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val workConstraints = Constraints.Builder()
            ...
            .setRequiredNetworkType(NetworkType.CONNECTED)  // 네트워크 필요 없음
            .build()
        ...
        WorkManager.getInstance(applicationContext).enqueueUniqueWork(ONE_TIME_WORK_1, ExistingWorkPolicy.REPLACE, oneTimeWork1)
        lifecycleScope.launch {
            WorkManager.getInstance(applicationContext).getWorkInfoByIdFlow(oneTimeWork1.id)
                .collect { workInfo ->
                    workInfo?.let {
