...
class AddEventDecorator(context: Context, color: Int, dates: ArrayList<CalendarDay>, duplicatesList: ArrayList<CalendarDay>) : ParentDecorator() {
    ...
    override fun shouldDecorate(day: CalendarDay?): Boolean {
        Log.i(TAG_C, "AddDecorator1 ${day?.date} ${dates.contains(day)}")
        if(duplicatesList.contains(day)) {
            Log.i(TAG_C, "AddDecorator1 적용 안된다.")
            return false
        } else {
